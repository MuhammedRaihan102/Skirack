...
const GoalForm = ({ initialData = {}, isEditing = false }) => {
  const { addGoal, updateGoal, loading: goalLoading, goals } = useGoalStore();
  ...
  // If editing, find the goal data
  useEffect(() => {
    if (isEditing && id && goals.length > 0) {
      const goalToEdit = goals.find(goal => goal.id === id);
      if (goalToEdit) {
        reset({
          title: goalToEdit.title,
          description: goalToEdit.description || '',
          target_date: new Date(goalToEdit.target_date).toISOString().split('T')[0],
          skill_id: goalToEdit.skill_id || '',
